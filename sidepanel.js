...
// ============================================================================
/* [ZeroLabs] 2026-09-23 2:35 PM - removed: the what's-new card and its flag */
...
/* [ZeroLabs] 2026-09-23 10:20 PM - added: setup reports what it is doing */
...
// a time. The dialog showed only "Connecting..." on a disabled button for all of
// it, which reads as frozen.
//
...
// including bringSidesTogether, which normal syncing also uses. With no reporter
// switched on the call does nothing, so an ordinary sync is unaffected.
let setupProgressReporter = null;
...
/* [ZeroLabs] 2026-09-23 11:30 PM - added: read a repository's bookmarks, and refuse a stranger's */
...
async function replaceLocalFromProjectStore(projectRef, remoteData, branch = 'main') {
  const replaced = await applyRemoteChangesToChrome(remoteData);
...
/* [ZeroLabs] 2026-09-23 10:50 PM - added: which cloud items are genuinely missing here */
...
// different folder - appears in `added` AND in `removed`. Creating everything in
// `added` therefore creates a second copy of a bookmark this device already has.
//
// The reconcile filtered that out. The join and the migration pull did not, and
// they create every item in `added` - which is where the duplicates came from.
// Both now call this, so the two cannot drift apart again.
//
// A URL on both lists is an edit, not an addition. A URL this device deleted is
// not wanted back. A folder is created only when a wanted bookmark is going into
// it, because attribution is URL-based and a folder has no URL of its own.
...
async function pullEverythingFromCurrentStore() {
  ...
  // time. The same filter the reconcile uses now decides what is really missing.
...
async function showSnippetSetup(mode = 'setup') {
  ...
  /* [ZeroLabs] 2026-09-23 10:20 PM - added: a progress panel for the slow actions */
  ...
  // the work runs. Closing would not stop it; it would only hide the progress of
  // a merge that is still writing bookmarks.
  let setupBusy = false;
  ...
  function onEscape(event) {
    /* [ZeroLabs] 2026-09-23 10:20 PM - edited: not while the work is running */
    if (setupBusy) return;
  ...
  function renderMigrationStart() {
    ...
    modal.querySelector('#startMigration').addEventListener('click', async () => {
      ...
      /* [ZeroLabs] 2026-09-23 10:20 PM - added: show the pull as it happens */
  ...
  /* [ZeroLabs] 2026-09-23 11:30 PM - added: the three answers for a repository that has bookmarks */
  ...
  async function renderExistingRepoChoice(ref, remoteData) {
    ...
    modal.querySelector('#optCloudWins').addEventListener('click', () => {
      // applyRemoteChangesToChrome asks twice itself before it removes anything
  ...
  function renderCreate() {
    ...
    modal.querySelector('#doCreate').addEventListener('click', async () => {
      ...
      /* [ZeroLabs] 2026-09-23 10:20 PM - added: show the create as it happens */
      const endProgress = beginSetupProgress();
      try {
  ...
  function wirePointAt(mode) {
    ...
    button.addEventListener('click', async () => {
      ...
      try {
        ...
        /* [ZeroLabs] 2026-09-23 10:20 PM - added: show the connect as it happens */
  ...
  if (snippetToken) {
    ...
    dialog.innerHTML = `
      ...
      <div style="display: flex; flex-direction: column; gap: 12px;">
        ...
        <!-- Which store you are connected to is the first thing you want when you
             open this dialog, and it was hidden behind the Cloud Sync Options
             toggle, which is collapsed whenever a store IS connected. It sits
             with the sync button now, above the divider, since both describe the
             current connection rather than offering an action. Rendered in both
             states, so a device connected to nothing says so here too. -->
    ...
    // Not awaited. The dialog is already usable, and a slow GitLab must not hold
    // it shut over a label. On failure the id simply stays, which is what the
    // dialog showed before this existed.
    if (snippetId) {
...
// is empty.
//
// `chrome.bookmarks.create` with no index APPENDS, so the renamed folder landed
// at the bottom of its parent on every other device while the original sat
...
// instead of at the end. Chrome inserts before it, and the old folder is pruned
// moments later, so the new one ends up in exactly the old one's slot.
//
...
async function resolveOrCreateFolderUnder(parentId, segments, options = {}) {
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    if (!match) {
      ...
      match = await chrome.bookmarks.create(details);
    }
...
async function showHeldPushDialog(fromUser = false) {
  ...
  // The dialog used to close on click and the work ran with no indicator at all.
  // An approved folder rename can carry thousands of bookmarks, and each one is a
  // search, an update, a move and a storage write. The dialog stays open and
  // becomes the progress surface, which also stops a second click starting the
  // same work twice.
  ...
  dialog.querySelector('#heldPushConfirm').addEventListener('click', async () => {
    ...
    // This used to write up to two entries per bookmark. MAX_CHANGELOG_ENTRIES is
        ...
        try {
          ...
          if (item.localPath !== item.remotePath && Array.isArray(item.remoteSegments)) {
            ...
            if (root) {
              ...
              let placeLike = null;
              if (node.parentId) {
                const [oldParent] = await chrome.bookmarks.get(node.parentId);
              ...
              const parentId = await resolveOrCreateFolderUnder(
                root.id, root.prefix.concat(item.remoteSegments), { placeLike });
              ...
              // The move restore used to resolve a path string back to a folder.
              // A bookmark title holding a slash makes that string ambiguous,
              // while the id is exact for as long as the folder exists. The path
              // is kept as the fallback and as what the user reads.
...
async function pruneEmptyFolderChain(startId) {
  ...
  for (let guard = 0; id && guard < 50; guard++) {
    ...
    if (!node.parentId || node.parentId === '0') return pruned;
...
async function applyFolderOrder(parentId, remoteChildren) {
  const localChildren = await chrome.bookmarks.getChildren(parentId);
...
async function applySnippetOrder(remoteRootNode, localRootNode) {
  ...
  const walk = async (remoteNode, localNode) => {
    ...
    for (const child of localChildren) {
      ...
      const [fresh] = await chrome.bookmarks.getSubTree(child.id);
...
// snippet format here, where Chrome's "Bookmarks Menu" and "Mobile Bookmarks"
// are roots of their own, and items only one side holds are ignored.
...
async function reconcileWithSnippet() {
  ...
  // A pending change alone is not enough. Adding a bookmark here after a reorder
  // on another device leaves a change pending while this device still holds the
  // old order.
  //
  // This runs before the content comparison below, and deliberately outside it:
  // a reorder produces no added, removed or modified entries at all, so the
  // early return for "no changes" would skip it. The same rule runs in
  // background.js for the push alarm and the five minute poll, so a manual sync
  // and an automatic one can never disagree.
  let publishOrder = false;
  try {
    const orderState = await chrome.storage.local.get(['snippet_push_pending', 'snippet_local_edited']);
    const localChangePending = orderState.snippet_push_pending === true;
    const movedHere = (orderState.snippet_local_edited || []).length > 0;

    const localAsSnippet = await chromeBookmarksToSnippetFormat(localTree);
  ...
  if (!hasChanges) {
    ...
    // Without this the sync button said "already in sync" and left the reorder
    // for the background push to carry up.
    if (publishOrder) {
      await syncToSnippet(true);
  ...
  if (removesFromSnippet.length > 0 || removesFromDevice.length > 0) {
    ...
    // list at 200 only made the counts wrong and, on the approve path, dropped
    // the work itself.
...
function showUnplaceableItemsDialog(skippedItems, createdCount) {
  ...
  dialog.innerHTML = `
    ...
    <!-- [ZeroLabs] 2026-09-23 1:06 AM - edited: the shared folder tree, not a flat list -->
  ...
  /* [ZeroLabs] 2026-09-23 1:06 AM - edited: the shared folder tree */
...
async function bringSidesTogether(items, silent = false, push = true) {
  ...
  try {
    ...
    /* [ZeroLabs] 2026-09-23 10:20 PM - added: count the slow part out loud */
...
/* [ZeroLabs] 2026-09-23 11:30 PM - added: the worker waits while a full replace runs */
// A full replace removes every bookmark, then recreates the cloud's one call at
// a time. If the background worker ran in the middle it would see a half-built
// tree, and the removals just recorded, and offer to delete the "missing"
// bookmarks from the cloud. This stamp tells runSnippetPush to wait. It carries
// a time, so a panel closed mid-replace cannot block syncing for ever.
...
async function applyRemoteChangesToChrome(remoteSnippetData, skipSnapshot = false) {
  ...
  return new Promise((resolve) => {
    const modal = document.createElement('div');
    /* [ZeroLabs] 2026-09-23 11:30 PM - edited: above the setup dialog */
    // Was 10000, under the setup dialog's 10001, so opening it from setup put the
    // warning behind the dialog that asked for it.
    ...
    dialog.querySelector('#confirmOverride').addEventListener('click', async () => {
      ...
      try {
        ...
        await chrome.storage.local.set({ [BULK_REPLACE_KEY]: Date.now() });
        ...
        /* [ZeroLabs] 2026-09-23 11:30 PM - added: count the recreate out loud */
        ...
        await setSnippetNeedsReconcile(false);
        /* [ZeroLabs] 2026-09-23 11:30 PM - added: the wipe and recreate are not your edits */
        ...
        await chrome.storage.local.remove(BULK_REPLACE_KEY).catch(() => {});
      }
    });
...
async function init() {
  ...
  /* [ZeroLabs] 2026-09-13 - added: look for published notices once the panel is up */
...
// ever goes up. The panel keeps the highest id it has shown and toasts everything
// above it, so rewording or deleting an old entry never re-notifies anyone. Only
// a new, higher id fires.
//
...
// panel; it simply tries again on the next open.
...
async function checkNotices() {
  ...
  const stored = await chrome.storage.local.get('bmz_notices_seen_id');
  ...
  const forThisClient = notices
    ...
    .filter(notice => {
      ...
      return notice.targets.includes('chrome');
    })
  ...
  // shutting the panel comes back next time and is written once, not twice.
  for (const item of unseen) {
    await addChangelogEntry('notice', 'notice', item.text, null, {});
  }
  await chrome.storage.local.set({ bmz_notices_seen_id: Number(newest.id) });
}
...
function renderBookmarks() {
  ...
  /* [ZeroLabs] 2026-09-23 2:35 PM - removed: the August 27 what's-new card */
...
/* [ZeroLabs] 2026-09-22 7:41 PM - added: everything inside a folder, at any depth (see also: Bookmark-Manager-Zero-Firefox/sidebar.js, Bookmark-Manager-Zero-Website/js/sidebar-adapted.js) */
...
function createFolderElement(folder) {
  ...
  // touchend version of this for the Android WebView; the side panel does not.
...
function setDragScrollSpeed(speed) {
  ...
  dragScrollFrame = setInterval(() => {
    ...
    // #bookmarkList carries scroll-behavior: smooth (sidepanel.html:677). A
...
// things: the wheel cannot scroll during a native drag (Chromium 41272694),
...
function renderFolderTree(selectElement, panel, options = {}) {
  ...
  const addRows = (nodes, depth) => {
    ...
    folders.forEach(folder => {
      ...
      /* [ZeroLabs] 2026-09-23 2:35 PM - added: the sidebar's folder icon, with its count */
...
async function expandMoveTreeTo(folderId) {
  ...
  for (let guard = 0; current && guard < 200; guard++) {
    let node;
    try {
      [node] = await chrome.bookmarks.get(current);
    } catch (error) {
      return;
    }
...
async function openAddBookmarkModal() {
  ...
  /* [ZeroLabs] 2026-09-23 12:52 AM - edited: the shared folder tree, not a flat list */
  ...
  if (lastUsedFolder && findBookmarkById(bookmarkTree, lastUsedFolder)) {
    ...
    const firstFolder = buildFolderList(bookmarkTree)[0];
...
async function openMoveToModal(item, isFolder) {
  ...
  /* [ZeroLabs] 2026-09-23 12:43 AM - edited: build the tree, not the flat list */
    ...
    if (entry.type === 'sync-apply') {
      ...
      for (const folder of prunedFolders.slice().reverse()) {
        ...
        try {
          ...
          const created = await chrome.bookmarks.create({ title: data.title, parentId, index });
      ...
      for (const item of renamed) {
        try {
          ...
          if (node && item.oldTitle) {
            await chrome.bookmarks.update(node.id, { title: item.oldTitle });
          } else {
            failed++;
          }
        } catch (error) {
...
async function bulkMoveItems() {
  ...
  /* [ZeroLabs] 2026-09-23 1:02 AM - edited: the folder tree, not a typed number */
...
changelogModal.addEventListener('keydown', (e) => {
  ...
  /* [ZeroLabs] 2026-09-22 10:32 PM - edited: back in the flow, exactly where it was */
  // Floating the bar was my idea and was never asked for, so it is gone. The
  // bar sits in the column above the list as it always has, and this only
  // toggles the class. The inline styles are cleared in case an older build
  // left any behind.
  // Matches the bmzBulkBarOut animation in sidepanel.html, so the bar retracts
  ...
  // Matches the bmzCheckboxOut animation in sidepanel.html
  ...
  multiSelectToggle.addEventListener('click', () => {
    ...
    if (multiSelectMode) {
      /* [ZeroLabs] 2026-09-23 2:35 PM - edited: red, because this button is the only way out */
  ...
  /* [ZeroLabs] 2026-09-22 7:17 PM - added: which input raised the gesture (see also: Bookmark-Manager-Zero-Website/js/sidebar-adapted.js, Bookmark-Manager-Zero-Firefox/sidebar.js) */
  ...
  function enterMultiSelectFromLongPress(itemEl) {
    ...
    if (!multiSelectMode) {
      ...
      /* [ZeroLabs] 2026-09-23 2:35 PM - edited: same red as the toggle handler uses */
      multiSelectToggle.style.background = 'var(--md-sys-color-error)';
      multiSelectToggle.style.color = '#ffffff';
      multiSelectToggle.setAttribute('aria-pressed', 'true');
...
(function () {
  ...
  // message, so on a narrow panel a long message had nowhere to go and overlapped
  // the "Scan All Bookmarks" label to its left. The CSS now lets the middle
  // section shrink; this makes the text fit inside whatever it gets.
  //
