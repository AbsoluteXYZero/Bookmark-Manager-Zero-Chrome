...
async function syncFromSnippet(silent = false) {
  ...
  try {
    ...
    if (silent) {
      console.log('[SyncFromSnippet] Differences found; leaving them to the background worker');
      return;
    }
...
async function bringSidesTogether(items, silent = false, push = true) {
  ...
  try {
    ...
    for (const item of ordered) {
      // A diff path ends with the item's own title, so its parent is everything
...
async function init() {
  ...
  const snippetIdResult = await chrome.storage.local.get(['bmz_snippet_id', 'snippet_local_version', 'snippet_last_sync', 'snippet_needs_reconcile']);
