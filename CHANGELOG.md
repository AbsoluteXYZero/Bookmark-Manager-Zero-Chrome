...
- **Mobile bookmarks keep one home** - Replacing this device's bookmarks with the cloud's put mobile bookmarks in a "Mobile Bookmarks" folder inside Other Bookmarks, while Chrome's own sync with your phone kept them in Chrome's Mobile bookmarks root. With both present, one of the two stopped reaching the cloud. They now always go in Chrome's Mobile bookmarks root.
...
- **Background syncing now recognises a full store** - The panel could spot the specific error GitLab returns for a snippet that has run out of room, but the panel is not where unattended syncing happens. The background worker read the status, threw away the message, retried twice more, gave up and raised a misleading card. It now names the failure, stops retrying something that can never succeed, and offers to move your bookmarks.
...
**New Features:**
- **Bookmarks now sync without opening BMZ** - Saving a bookmark with the star button, Ctrl+D, or Chrome's own bookmark manager used to go unnoticed until the next time you opened the side panel, because the only thing watching for changes lived in the panel itself. That watcher now runs in the extension's background, so a bookmark you save while browsing reaches your snippet about thirty seconds later with BMZ never opened. Bookmarks added on your other devices arrive here the same way, checked for every five minutes.
...
**Bug Fixes:**
- **"Replace Remote Snippet with Local" wrote an empty snippet** - Choosing it replaced your snippet with a file containing no bookmarks at all rather than with the bookmarks on this device. The conversion of your bookmarks was never waited for, so an unfinished result was uploaded in place of the real one. Anyone who used this option had their snippet emptied at that moment.
- **"Merge Bookmarks" never merged anything** - It reported success and uploaded the snippet back to itself unchanged. The same missing step meant the merge received nothing to merge and quietly did nothing. This had never worked in any released version.
...
- **Bookmark titles containing markup broke the sync comparison** - A title with characters that look like HTML rendered as formatting in parts of the sync dialog rather than as text.
...
- **Blocklists no longer download when safety checking is off** - As a result of the fault above, the security database was fetched when Chrome started, again on every extension reload or update, and again before every scan, for a feature that may well have been switched off - all ten lists, repeatedly, for data nothing would ever read. The download now checks the setting, and picks the lists back up as soon as you turn safety checking on again.
- **The status bar no longer gets stuck on a finished scan** - It could sit showing "Scanning: 7/7" after everything had finished, with nothing left to clear it. Progress messages were being written to the bar on a short delay while the reset to "Ready" happened immediately, so a late message could arrive after the reset and overwrite it. The full rescan was also updating the bar without registering as an activity at all, which meant anything else finishing would blank out its progress mid-scan. All of it now goes through one place, so the bar always reflects what is genuinely still running.
...
**New Features:**
- **Setup now offers a snippet you already have** - After entering your GitLab token for the first time, BMZ checks for existing bookmark snippets and offers to use one directly. Previously it dropped you into the sync settings, where finding an existing snippet meant knowing to open Select Existing Snippet and browse for it. Setting up a second device is now a single click.
...
- **Token storage options explain themselves** - Local and Supabase each carry an information icon with a full explanation of what that choice means for your token when it renews. The previous version crammed the same detail into permanently visible text beside each option.
...
**New Features:**
- **Quick Access** - Pin the bookmarks you use most to a section at the top of the side panel. Right-click any bookmark and choose "Add to Quick Access", or drag one onto the section. Pinned entries are mirrors, not copies: the same bookmark, shown in a second place. Delete the bookmark from its real folder and the pin disappears with it, including when you delete it in Chrome's own bookmark manager. Removing a pin only unpins it and never touches the underlying bookmark, so there is no way to destroy a bookmark from the Quick Access section.
...
- **Display toggles** - Both sections can be shown or hidden independently from the Display Options button in the toolbar. Unlike the existing display toggles, these two persist across side panel reopens. Hiding one gives the other the full width.
...
- **Skip the merge dialog when bookmarks already match** - Connecting to an existing snippet always asked you to choose a merge strategy, even when your bookmarks were already identical to it. BMZ now compares the two first and connects quietly when there is nothing to reconcile. This works even when the snippet was last written by Firefox.
...
**Bug Fixes:**
- **Header buttons no longer overlap the title/subtitle** - The GitLab, sync, logout, and settings buttons now sit in their own reserved space in the header instead of floating over it, and the title and subtitle automatically scale down to fit the remaining width on a single line. Fixes the buttons covering the title or subtitle at narrow side panel widths and after signing in to GitLab (which adds buttons to the row).
...
### v4.7 - Supabase Token Sync & GitLab OAuth
...
- **Move to... Context Menu** - Right-click any bookmark or folder and select "Move to..." to relocate it via a modal folder picker
  ...
  - Protects built-in Chrome root folders (Bookmarks Bar, Other Bookmarks)
