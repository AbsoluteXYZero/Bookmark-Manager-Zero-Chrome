...
/* [ZeroLabs] 2026-09-23 8:30 PM - added: a reorder is a change worth pushing (see also: Bookmark-Manager-Zero-Firefox/background.js, Bookmark-Manager-Zero-Website/js/storage/sync-manager.js) */
...
// back into the slots that remain. Chrome's "Bookmarks Menu" and "Mobile
// Bookmarks" folders inside Other Bookmarks are never matched, so they stay put.

/**
 * Put one Chrome folder's children into the order the snippet holds.
 *
...
async function applyWorkerFolderOrder(parentId, remoteChildren) {
  ...
  // in the list, and Chrome's remove-then-insert needs no index correction.
...
/**
 * Walk the snippet and put every Chrome folder that differs into its order.
 *
...
async function applyRemoteOrderInWorker(remoteData) {
  ...
  const walk = async (remoteNode, localId) => {
    ...
    const localChildren = await chrome.bookmarks.getChildren(localId);
  ...
  for (const key of Object.keys(remoteRoots)) {
    ...
    // The snippet's menu root lives inside Other Bookmarks here, in a folder
    // of its own name
    let localId = root.id;
...
async function recordLocalBookmarkEdit(id, explicitUrl) {
  try {
    /* [ZeroLabs] 2026-09-23 1:00 AM - edited: a folder is recorded through its contents */
    ...
    await chrome.storage.local.set({ snippet_local_edited: Array.from(list) });
  } catch (error) {
...
async function runSnippetPush() {
  ...
  /* [ZeroLabs] 2026-09-23 11:30 PM - added: never run in the middle of a full replace */
  // The panel is removing every bookmark and recreating the cloud's. A run now
  ...
  // older than ten minutes is from a panel closed mid-replace and is ignored.
  const { bmz_bulk_replace_started: replaceStarted } =
    await chrome.storage.local.get('bmz_bulk_replace_started');
  if (replaceStarted && Date.now() - replaceStarted < 10 * 60 * 1000) {
    console.log('[CloudSync] A full replace is running in the panel, waiting');
    chrome.alarms.create(SNIPPET_PUSH_ALARM, { delayInMinutes: SNIPPET_PUSH_DELAY_MIN });
    return;
  }
  ...
  try {
    ...
    const pendingState = await chrome.storage.local.get('snippet_push_pending');
    ...
    /* [ZeroLabs] 2026-09-22 6:54 PM - edited: store every item, cap only the display (see also: sidepanel.js) */
